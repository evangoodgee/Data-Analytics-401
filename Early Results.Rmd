...
MJRdata = read_csv("Data/College_Scorecard_Raw_Data_05192025/FieldOfStudyData1718_1819_PP.csv")
...
Class = read_excel("Data/IPEDS_2014-15_Final/HD2014.xlsx")
Classification = Class %>% select(UNITID, CCBASIC)
...
EducationData = merge(Merged, Classification, by  = "UNITID") %>% 
  select(UNITID, INSTNM, ICLEVEL, CCBASIC, everything(), -SecondaryTranscript, -SecondaryGPA, -TestScores, -PRGMOFR, -MD_EARN_WNE_P6, -MD_EARN_WNE_P8, -MD_EARN_WNE_P10)  %>% 
  filter(ICLEVEL == 1) %>% 
